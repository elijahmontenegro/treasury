...
Fifty real label approvals from the TTB public registry are the population this was built
against, and every number below was measured rather than argued. The arc, in ten lines.

**It began as decoding.** A label was taken to be a document: cut the ink into glyphs, learn the
label's own alphabet by aligning the statutory warning to its known text, then spell each claim in
that alphabet and match by distance. Eighteen steps went into making it work — fused glyphs split,
partial alphabets accepted per character, a digit classifier, a contrastive glyph encoder, an
evaluation protocol rebuilt when it was found to leak, and determinism made a test after the
engine was caught disagreeing with itself.

**Step 18a measured the stage everything rested on** and found it correct on 0.39 of characters,
and on 0.25 through a camera channel. Nothing above a segmentation that wrong can be fixed by
tuning what sits on top of it, so the reading engine was retired and replaced: a label is scene
text, and a pretrained detector and recogniser read whole regions instead.

**The judging never changed in principle**, and it is the part that was right from the start. What
did change is that every rule it now carries was forced by a specific false assertion, named with
the label that produced it — seven of them by the end. The fifty went from 63 verified claims to
156 of the 192 they carry, and **precision stayed at 1.00 across 550 labels: 2,572 verifications
and not one assertion a label does not bear out.** The five-second latency requirement, carried and
unmet since step 3, was met at step 29b.

**Read it as a record of measurements, not a description of a design.** The tables that were later
corrected are kept where they were made, with what was wrong with them, because a build that only
shows the numbers that survived is not showing its reasoning.

## Contents

<!-- contents: generated by out/toc.py from the headings below -->

**Before the record — what this is**

- [What this engine does](#what-this-engine-does)
- [What shipped](#what-shipped)
- [Pipeline](#pipeline)
- [Evaluation protocol](#evaluation-protocol)
- [How to read the record below](#how-to-read-the-record-below)

**Part one — the engine that read by cutting glyphs (steps 1 to 18, retired)**

- [The problem as decoding (as stated through step 18)](#the-problem-as-decoding-as-stated-through-step-18) — the assumption the first eighteen steps rested on: a label is a document whose ink can be cut into glyphs.
- [The pipeline as it stood](#the-pipeline-as-it-stood) — the stages, threshold to alignment to spelling, as they were before the pivot.
- [The glyph code](#the-glyph-code) — how a glyph became bits, and the measurements that chose the encoding.
- [The evaluation protocol as it stood](#the-evaluation-protocol-as-it-stood) — held-out faces and half A against half B, before step 8a rebuilt the partition.
- [Results: 500 held-out labels, 2026-09-03](#results-500-held-out-labels-2026-09-03) — the first full table, free-text recall by claim on faces the models never saw.
- [Alphabet robustness, half B re-run, 2026-09-03](#alphabet-robustness-half-b-re-run-2026-09-03) — `no_alphabet` after fused-glyph splitting and per-character acceptance: 28.8% to 6.4%, inside the target.
- [Digits: a classifier, not an encoder (amendment step 2, 2026-09-03)](#digits-a-classifier-not-an-encoder-amendment-step-2-2026-09-03) — a 52k-parameter digit network on held-out faces: 96.7%, short of the 98% gate, and recorded as short.
- [Numeric fields are read, not enumerated (amendment step 3, 2026-09-03)](#numeric-fields-are-read-not-enumerated-amendment-step-3-2026-09-03) — reading the number and validating the value: median 2.7 s, p95 4.1 s, alcohol recall 0.31 to 0.59.
- [Real labels: ten from the COLA registry (amendment step 4, 2026-09-03)](#real-labels-ten-from-the-cola-registry-amendment-step-4-2026-09-03) — first contact with real images, and it went badly: nine of ten learned no alphabet.
- [Step 4b: mechanisms for what the real labels showed (2026-09-03)](#step-4b-mechanisms-for-what-the-real-labels-showed-2026-09-03) — what the ten forced, re-measured from the step 3 commit with both numbers: 1 of 10 to 6 of 10.
- [Step 5a: the generator learns the real conventions (2026-09-03)](#step-5a-the-generator-learns-the-real-conventions-2026-09-03) — the ten's conventions put into the corpus at their measured rates; cross-face gap about 0.1, vertical labels 0.00.
- [Step 5b: the contrastive encoder on cross-face pairs (2026-09-04)](#step-5b-the-contrastive-encoder-on-cross-face-pairs-2026-09-04) — a 108k-parameter glyph encoder closed the cross-face gap, class 0.36 to 0.77; the real ten reached 12 verified.
- [Step 6a: orientation by detection (2026-09-04)](#step-6a-orientation-by-detection-2026-09-04) — orientation measured rather than searched: the vertical real label 14.5 s to 8.7 s, still over the 5 s target.
- [Step 6b: encoder coverage for the tail (2026-09-04)](#step-6b-encoder-coverage-for-the-tail-2026-09-04) — retrained on styled glyphs, and found the metric wrong: a false mismatch had been scored as a miss.
- [Step 7a: numeric reads complete or undecided (2026-09-04)](#step-7a-numeric-reads-complete-or-undecided-2026-09-04) — twelve false mismatches traced to region boundaries cutting a number in half, not to the classifier.
- [Step 8a: the evaluation protocol (2026-09-04)](#step-8a-the-evaluation-protocol-2026-09-04) — font families partitioned once at the source and the models retrained; it exposed that the engine was not deterministic.
- [Step 8b: digits from the image (2026-09-04)](#step-8b-digits-from-the-image-2026-09-04) — the label's own printed fill statement taught as a second known string.
- [Step 8c: which domain rules earn their place (2026-09-04)](#step-8c-which-domain-rules-earn-their-place-2026-09-04) — four domain rules removed one at a time; each kept only where removal cost measured accuracy.
- [Step 8d: what the learned parts carry (2026-09-04)](#step-8d-what-the-learned-parts-carry-2026-09-04) — the table three ways: geometry alone, plus image-taught digits, and the whole system.
- [Step 9a to 9c: determinism, identity, and a pipeline (2026-09-04)](#step-9a-to-9c-determinism-identity-and-a-pipeline-2026-09-04) — determinism made a test, the model hashes put in the build identity, and the leak check made a CI failure.
- [Step 9d: fifty real labels (2026-09-04)](#step-9d-fifty-real-labels-2026-09-04) — the fifty assembled with provenance, and the two tables set side by side with a stated transfer finding.
- [Step 10a: separating text from artwork (2026-09-05)](#step-10a-separating-text-from-artwork-2026-09-05) — text separation measured on real artwork with evidence images, naming the labels that still fail.
- [Step 10b: the corpus rebuilt from the population (2026-09-05)](#step-10b-the-corpus-rebuilt-from-the-population-2026-09-05) — every generator parameter redrawn from the fifty with its source recorded; the transcribed polarity count was wrong against the pixels.
- [Step 10c: retuning on the corpus that models the population (2026-09-05)](#step-10c-retuning-on-the-corpus-that-models-the-population-2026-09-05) — 33 constants retuned with precision as a constraint; several given back, and a first run reporting all of them insensitive turned out to be broken.
- [Step 11a: vertical warnings, by the stage that fails (2026-09-05)](#step-11a-vertical-warnings-by-the-stage-that-fails-2026-09-05) — the causes counted on the fourteen labels with a vertical warning: eight learn an alphabet, six do not, two choose the wrong orientation.
- [Step 11b: cross-face reach, measured (2026-09-05)](#step-11b-cross-face-reach-measured-2026-09-05) — real recall split by whether the claim sits in the warning's own face, transcribed by eye to avoid circularity.
- [An audit of the verifications, and one false assertion (2026-09-05)](#an-audit-of-the-verifications-and-one-false-assertion-2026-09-05) — every verified claim read against its image, which found one assertion the label did not carry.
- [Step 12a: ground truth is what the label prints (2026-09-05)](#step-12a-ground-truth-is-what-the-label-prints-2026-09-05) — the real set rescored against the transcribed printed text: precision had been 0.94, not 1.00.
- [Step 12b: the form gap, decided on merit (2026-09-05)](#step-12b-the-form-gap-decided-on-merit-2026-09-05) — whether a filed name may verify against a printed variant, argued as a rule about the domain rather than a patch to reach a number.
- [Step 12c: alphabet acceptance considers coverage (2026-09-05)](#step-12c-alphabet-acceptance-considers-coverage-2026-09-05) — coverage added to acceptance, so a partial alphabet cannot pass on a handful of characters.
- [Step 13a: the cache key, and a determinism test that varies the input (2026-09-06)](#step-13a-the-cache-key-and-a-determinism-test-that-varies-the-input-2026-09-06) — an inexact cache key fixed, and determinism extended to vary claim order and composition; the old test would not have caught it.
- [Step 13b: where the same-face distance comes from (2026-09-06)](#step-13b-where-the-same-face-distance-comes-from-2026-09-06) — the distance decomposed, with evidence images of the spelled codeword beside the region it was compared to.
- [Step 14a: the learned alphabet against the font set (2026-09-06)](#step-14a-the-learned-alphabet-against-the-font-set-2026-09-06) — the template source benched both ways and decided by measurement rather than by argument.
- [Step 14b: orientation as a measurement (2026-09-06)](#step-14b-orientation-as-a-measurement-2026-09-06) — nothing required of the uploaded image: upright, rotated, inverted and skewed all handled by measuring.
- [Step 14c: re-deciding the two learned components (2026-09-06)](#step-14c-re-deciding-the-two-learned-components-2026-09-06) — neither learned part kept by inheritance; each made to earn its place again.
- [Step 14d: one retuning pass (2026-09-07)](#step-14d-one-retuning-pass-2026-09-07) — found that eight of 10c's adopted constants had never been written into the code, and that a 24-label subset can mislead.
- [Step 15a: the apparatus, tested (2026-09-07)](#step-15a-the-apparatus-tested-2026-09-07) — a test that every adopted constant is the value the engine runs, and that an override reaches the path it names.
- [Step 15b: the separation ceiling (2026-09-07)](#step-15b-the-separation-ceiling-2026-09-07) — the curve, and whether rank rather than radius is the sounder decision rule.
- [Step 16a: the free-text radius at 0.15, measured and given back (2026-09-07)](#step-16a-the-free-text-radius-at-015-measured-and-given-back-2026-09-07) — 0.15 adopted and then given back: it verified a brand on two labels printing another.
- [Step 16b: the same question for the numeric radius (2026-09-07)](#step-16b-the-same-question-for-the-numeric-radius-2026-09-07) — the same curve for numbers, ending in a statement that no value was free.
- [Step 17b: where the alphabet still dies (2026-09-07)](#step-17b-where-the-alphabet-still-dies-2026-09-07) — 11a's diagnosis repeated on the engine as it then stood.
- [Step 17a: position as evidence (2026-09-07)](#step-17a-position-as-evidence-2026-09-07) — whether a region's size and position separate a brand from a producer, and whether they would refuse the two known false brands without cost.
- [Step 18a: segmentation, measured directly (2026-09-07)](#step-18a-segmentation-measured-directly-2026-09-07) — the measurement that ended part one: the stage everything rested on correct on 0.39 of characters, 0.25 through a channel, fusion dominating.
- [Step 18b: what the separation step discards (2026-09-07)](#step-18b-what-the-separation-step-discards-2026-09-07) — evidence images of discarded regions holding claim text, and whether display type can be admitted without admitting artwork.

*Part one, as it was left*

- [What the numbers said (step 6)](#what-the-numbers-said-step-6) — part one's closing numbers, kept as they stood.
- [Limits as they stood (step 6)](#limits-as-they-stood-step-6) — part one's closing limits, kept as they stood.

**The pivot (step 19a)**

- [The pivot: the reading engine is replaced (step 19a, 2026-09-07)](#the-pivot-the-reading-engine-is-replaced-step-19a-2026-09-07) — what was deleted and why, with 18a as the evidence: a label is scene text, not a document.

**Part two — the engine that reads whole regions (steps 19 to 30)**

- [Step 19b: detection and recognition (2026-09-07)](#step-19b-detection-and-recognition-2026-09-07) — the new reader on the fifty: 1,982 regions, median 1.0 s, and 0.73 of printed claim text recovered.
- [Step 19c: verification over recognised text (2026-09-07)](#step-19c-verification-over-recognised-text-2026-09-07) — the decision layer rebuilt over readings rather than bit codes: 63 of 191 verified, precision 1.00.
- [Step 20a: where the fifty's unverified claims are lost (2026-09-07)](#step-20a-where-the-fiftys-unverified-claims-are-lost-2026-09-07) — the 128 named by cause, with no fix in the step.
- [Step 20b: a number is taken from beside another statement (2026-09-07)](#step-20b-a-number-is-taken-from-beside-another-statement-2026-09-07) — a numeric span delimited by its own unit: 51 claims.
- [Step 20c: the loop, and where it stops (2026-09-08)](#step-20c-the-loop-and-where-it-stops-2026-09-08) — repeated to the largest cause the engine cannot address, and stopped one step too early, as step 21 found.
- [Step 21a: the claims the whole-run rule refuses, judged one by one (2026-09-08)](#step-21a-the-claims-the-whole-run-rule-refuses-judged-one-by-one-2026-09-08) — the 21 quoted one at a time and split into rightly and wrongly refused.
- [Step 21b: what delimits a name (2026-09-08)](#step-21b-what-delimits-a-name-2026-09-08) — punctuation, a digit or an edge, and never a space: the rule that admits the wrongly refused without readmitting the false brands.
- [Step 21c: the twenty-six never read (2026-09-08)](#step-21c-the-twenty-six-never-read-2026-09-08) — a second detector and passes at other scales; which labels stay unread, and why a stacked logotype is one of them.
- [Step 21d: reading the page both ways up (2026-09-08)](#step-21d-reading-the-page-both-ways-up-2026-09-08) — both crop rotations read and the surer kept.
- [Step 23a: where the fifty-five are lost, on the engine as it stands (2026-09-08)](#step-23a-where-the-fifty-five-are-lost-on-the-engine-as-it-stands-2026-09-08) — the diagnosis re-run rather than carried forward, with p95 reported beside the median.
- [Step 23b: a run is a chain, not a slice of the reading order (2026-09-08)](#step-23b-a-run-is-a-chain-not-a-slice-of-the-reading-order-2026-09-08) — adjacency decided on merit: what relation justifies joining two detections, and what it would wrongly join.
- [Step 23c: the next cause, and where it stops (2026-09-08)](#step-23c-the-next-cause-and-where-it-stops-2026-09-08) — the responsibility phrases completed from the regulation, and the stopping point stated with the measurement that shows it.
- [Step 24a: the second detection pass, spent where it is needed (2026-09-08)](#step-24a-the-second-detection-pass-spent-where-it-is-needed-2026-09-08) — the turned pass made conditional: recall unchanged, median and p95 before and after.
- [Step 24b: whether position or size separates the nine (2026-09-08)](#step-24b-whether-position-or-size-separates-the-nine-2026-09-08) — measured, and nothing separates them without refusing a true verification; said so and stopped.
- [Step 25a: the ceiling, and a word the regulation says is one word (2026-09-08)](#step-25a-the-ceiling-and-a-word-the-regulation-says-is-one-word-2026-09-08) — whisky and whiskey as one class under the regulation; the radius re-measured without that label pinning it and adopted at 0.14.
- [Step 25b: a second opinion on near misses, measured and not adopted (2026-09-08)](#step-25b-a-second-opinion-on-near-misses-measured-and-not-adopted-2026-09-08) — priced and refused: two claims for a p95 that would not hold.
- [Step 25c: the printed forms the enumeration lacks (2026-09-08)](#step-25c-the-printed-forms-the-enumeration-lacks-2026-09-08) — the enumerations completed from the regulation text rather than from the labels.
- [Step 26a: latency measured three times, then profiled (2026-09-08)](#step-26a-latency-measured-three-times-then-profiled-2026-09-08) — the first stage-by-stage profile: deciding scales with the run count, not the box count, and a p95 under 5 s costs three claims.
- [Step 26c: whether the abutting word separates the nine (2026-09-08)](#step-26c-whether-the-abutting-word-separates-the-nine-2026-09-08) — the distinction is real and exactly inverted, so the rule would admit none of the nine and all three it exists to refuse.
- [Step 26b: the middle path on recogniser damage (2026-09-08)](#step-26b-the-middle-path-on-recogniser-damage-2026-09-08) — a larger recogniser on near misses: two claims, p95 6.8 s, not adopted.
- [Step 26d: what the corpus lacks (2026-09-08)](#step-26d-what-the-corpus-lacks-2026-09-08) — the corpus models the shape of hard cases but not their frequency; text arriving in pieces is what it never produced.
- [Step 27a: the 24, named one at a time (2026-09-08)](#step-27a-the-24-named-one-at-a-time-2026-09-08) — listing before grouping, which caught a probe ignoring adjacency and a count scored on a stale diagnostic.
- [Step 27b: whether the label prints the filed value alone somewhere else (2026-09-08)](#step-27b-whether-the-label-prints-the-filed-value-alone-somewhere-else-2026-09-08) — no, on either side; and the discovery that the two corpus guards had stopped guarding.
- [Step 27c: the residual recogniser damage, and a confusion-aware distance (2026-09-08)](#step-27c-the-residual-recogniser-damage-and-a-confusion-aware-distance-2026-09-08) — the confusion set taken from the engine's own evidence, and an argument by arithmetic that the measurement is complete.
- [Step 27d: the corpus splits its statements (2026-09-08)](#step-27d-the-corpus-splits-its-statements-2026-09-08) — claims needing a chain 0.25 to 0.52, inside tolerance; runs per detection missed, because it measures crowding rather than splitting.
- [Step 28a: the guard, made real (2026-09-08)](#step-28a-the-guard-made-real-2026-09-08) — a rendered label the test cannot pass vacuously, in CI; and the refusal turns out to have no margin on the boundary path.
- [Step 28b: a diagnostic that says which engine wrote it (2026-09-08)](#step-28b-a-diagnostic-that-says-which-engine-wrote-it-2026-09-08) — provenance on every diagnostic and a checker that refuses a stale one, with 26c's own file as the case.
- [Step 28c: the confusion-aware distance adopted (2026-09-08)](#step-28c-the-confusion-aware-distance-adopted-2026-09-08) — seven claims, every one a confusion in the set; and a per-cell function call that trebled the p95 before it was inlined.
- [Step 28d: the boundary rule on a chained run (2026-09-08)](#step-28d-the-boundary-rule-on-a-chained-run-2026-09-08) — it produced a false assertion on the first run, which forced the seventh precision rule: a name taken from inside a longer reading must be the whole name.
- [Step 29a: parallelism inside one verification (2026-09-09)](#step-29a-parallelism-inside-one-verification-2026-09-09) — byte-identical across 3,814 claims; the median halves and the p95 barely moves.
- [Step 29b: bounded distance (2026-09-09)](#step-29b-bounded-distance-2026-09-09) — two exact cuts take the p95 to 3.9 s at four cores and the five-second requirement is met; the tail was a quantity of comparisons, not a shortage of cores.
...
### Step 30f: the release, and the deployed shape (2026-09-09)

The container is cut by `goreleaser release`, which passes the commit and its date into the
Dockerfile as build arguments and writes them into the OCI labels as well, so the image says which
source it came from in two places that have to agree. It does not build the binary, and the reason
is worth saying rather than leaving to be discovered: the reader calls ONNX Runtime through cgo, so
the binary must be linked against that library on the platform it will run on, and the usual shape
— cross-compile on the host, wrap the result in a thin image — cannot produce it from this
workstation. The Dockerfile builds it inside a Linux image, as it has since step 30's second item.

Two things were found by running it rather than by reading about it. GoReleaser builds in a
temporary directory holding the Dockerfile and nothing else, so `COPY . .` found an empty context
and the first attempt failed; the context is now named in the configuration, which is better than
it sounds, because it states positively the whole of what the image is built from where
`.dockerignore` states the same set by subtraction. And the SBOM attestation needs a builder the
default Docker driver does not provide, which the release names as a prerequisite instead of
silently dropping.

**The changelog is grouped by what a reader of this project would look for** — what reads the
label, what decides, what serves it, what was measured — because the history is one gated step per
commit and the subjects already carry that grouping. The first set of patterns left 38 of 87
commits ungrouped, among them the digit classifier and the numeric read, so they were widened
against the actual subjects until what falls through is what genuinely belongs nowhere.

**The deployed shape.** One instance, two vCPU, request-based billing so the CPU is charged only
while a request is in flight, startup CPU boost, and **a concurrency of one**. That last is not the
default and is the one choice worth defending: latency here has meant, since step 29, *one
verification at a time with every core available to it*. At a concurrency above one and a single
instance, two callers arriving together each get half the machine and both see roughly twice the
latency, so the deployed number would stop meaning what the local number means. One instance
serving one request at a time is a pilot shape, and it is what makes the two columns below
