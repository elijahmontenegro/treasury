...
### Step 27c: the residual recogniser damage, and a confusion-aware distance (2026-09-08)

Step 25a widened the free-text radius from 0.07 to 0.14, so some of the single-character errors
step 23c reported as out of reach now verify. What remains is here, taken from the engine's own
evidence rather than from a fresh comparison: since step 25b a claim that decides nothing names
the reading it looked at and the spelling it compared that reading to, so the near misses can be
re-scored under a different metric without re-running anything.
...
**The finding.** Two of the three substitutions the amendment named appear in the residual —
`O` returned as `0` on 0034's `12 FL. 0Z.`, and `G` returned as `C` on 0050's `CRaPEViNE` — and
`1` for `I` does not appear at all. Three more appear that are **not** confusions of shape: `E`
read as `A` on the three Owl's Brew labels, and `Y` as `P` and `L` as `S` on 0006's stacked
logotype. Those are a recogniser failing on display type, not two characters that look alike, and
putting them in the set would be fitting to the labels the set is scored on — the same objection
that stopped step 23c choosing a radius between 0.071 and 0.077.

**The measurement, on the shape-confusable set only: five claims admitted across the three sets
and not one claim the labels do not carry.** On the fifty it is 0034's fill, `12 FL. 0Z.` against
`12 FL OZ`, from 0.167 to 0.083. The other four are the corpus, which is the first time in this
build that the corpus has produced more of a kind of case than the fifty: `Portand, 0regow 97209`
for an Oregon address, `15% A1c./Vol.`, `39: 5% ABV`, `145% ABV`.

**And the measurement is complete at that cost, by arithmetic rather than by luck.** The evidence
window is twice the radius, so a claim whose nearest reading sat further away has no near miss
recorded and could not have appeared. With every edit costing at least a half, a re-scored
distance is at least half the original, so a claim beyond twice the radius stays beyond the
radius: the window and the bound are the same number. At zero cost that argument fails — a
distance can fall to zero however large it was — and zero cost is the wrong rule anyway, since it
says `O` and `0` are the same character, which inside a numeric figure is exactly the equivalence
step 19c added a rule to forbid.

**Nothing is adopted here**, because this step's gate is the confusion set and the measurement,
and adopting a distance is an engine change that has to be measured on all three sets with
